...
#line 2 "..\\..\\Src\\core\\eepromDriver.c"
...
#line 3 "..\\..\\Src\\core\\eepromDriver.c"
...
#line 4 "..\\..\\Src\\core\\eepromDriver.c"
...
#line 5 "..\\..\\Src\\core\\eepromDriver.c"
...
tdStatus EEPROM_DRIVER_write(tdSpiDevice *_pDevice, uint8_t *_pBuff, uint16_t _size)
	{
	tdStatus status;
	uint8_t writeEnableCommand[ 1 ] = { (uint8_t)WriteEnable };
	uint8_t writeEnableReadBack[ 1 ] = { 0x00 };
	...
	do
		{
		status = EEPROM_DRIVER_dataTransmition(_pDevice, writeEnableCommand, writeEnableReadBack, 1); 
		} while( status == Busy );	
	...
	if( status == Ok )
		...
		do
			{
			status = EEPROM_DRIVER_dataTransmition(_pDevice, _pBuff, rxBuff, _size); 
			} while( status == Busy );	
		}
...
tdStatus EEPROM_DRIVER_writeByte(tdSpiDevice *_pDevice, uint16_t _address, uint8_t _byte)
	...
	return EEPROM_DRIVER_write(_pDevice, txBuff, 1 + 2 + 1 );
	}
...
tdStatus EEPROM_DRIVER_writeBuffer(tdSpiDevice *_pDevice, uint16_t _address, uint8_t *_pBuffer, uint16_t _size)
	...
	uint16_t transmissionSize = 1 + 2 + _size;
...
tdStatus EEPROM_DRIVER_read(tdSpiDevice *_pDevice, uint16_t _address, uint8_t *_pBuff, uint16_t _size)
	...
	do
		{
		status = EEPROM_DRIVER_dataTransmition(_pDevice, txBuff, _pBuff,  transmissionSize); 
		} while( status == Busy );	
...
tdStatus EEPROM_DRIVER_readByte(tdSpiDevice *_pDevice, uint16_t _address, uint8_t *_byte)
	{
	tdStatus status;
	uint8_t rxData[ 1 + 2 + 1 ];

	status = EEPROM_DRIVER_read(_pDevice, _address, rxData, 1 );
...
tdStatus EEPROM_DRIVER_eraseRange(tdSpiDevice *_pDevice, uint16_t _addressFrom, uint16_t _addressTo)
	...
	uint16_t transmissionSize = 1 + 2 + size;
...
tdStatus EEPROM_DRIVER_format(tdSpiDevice *_pDevice)
	...
	for( addressFrom = 0x0000, addressTo = 1024; addressFrom <= 0x100000 - 1024; addressFrom += 1024, addressTo += 1024 )
		{
		if( (addressTo < 0x100000 - 1) == 0 )
			addressTo = 0x100000 - 1;

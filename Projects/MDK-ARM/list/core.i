...
#line 3 "..\\..\\Src\\core\\core.c"
#line 4 "..\\..\\Src\\core\\core.c"
...
#line 5 "..\\..\\Src\\core\\core.c"
#line 6 "..\\..\\Src\\core\\core.c"
#line 1 "..\\..\\Inc\\crc.h"
...
#line 8 "..\\..\\Src\\core\\core.c"
#line 9 "..\\..\\Src\\core\\core.c"
...
#line 43 "..\\..\\Inc\\usb_device.h"
#line 44 "..\\..\\Inc\\usb_device.h"
...
#line 16 "..\\..\\Inc\\adcCore.h"
#line 17 "..\\..\\Inc\\adcCore.h"
#line 18 "..\\..\\Inc\\adcCore.h"
#line 19 "..\\..\\Inc\\adcCore.h"
#line 20 "..\\..\\Inc\\adcCore.h"
...
#line 16 "..\\..\\Inc\\timCore.h"
#line 17 "..\\..\\Inc\\timCore.h"
#line 18 "..\\..\\Inc\\timCore.h"
...
#line 41 "..\\..\\Src\\core\\core.c"
	...
	tdUsbConectionStatus isUsbConnected(void)
		...
		while( vBusChecks-- )
			{
			if( HAL_GPIO_ReadPin(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0000)), ((uint16_t)0x0001)) )
				{
				vBus++;
				}
			}

		return ( vBus > 0xFF / 2 ) ? usbNotConnect : usbConnect;
		}

#line 103 "..\\..\\Src\\core\\core.c"
...
void CORE_setSignalOk(uint8_t _state)
	{
	GPIO_PinState state;

	state = _state ? GPIO_PIN_SET : GPIO_PIN_RESET;




	HAL_GPIO_WritePin(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1400)), ((uint16_t)0x2000), state); 
...
void CORE_init(void)
	...
	usbConectionStatus = isUsbConnected();
...
void CORE_threadEraseMainMemory(void)
	...
	if( (mainMemoryErase.erasingBlock < 4096 ) == 0 )
		{
...
void CORE_threadClearBadBlocks(void)
	...
	FILE_SYSTEM_clearBadBlocksList_( clearBadBlocks.counter, 16 );

	clearBadBlocks.counter += 16;
	if( (clearBadBlocks.counter < (4096 / 8)) == 0 )
		{
...
tdChannelState CORE_holtIsChannelActive(tdArinc *_pArinc, tdArincChannel *_pChannel, uint8_t _channelNumber)
	...
	static uint8_t triggerOffCh1Prev = 0;
	static uint8_t triggerOffCh2Prev = 0;
	uint8_t triggerOffCh1;
	uint8_t triggerOffCh2;
	uint32_t msTick;



#line 246 "..\\..\\Src\\core\\core.c"
	if( ( (_pArinc->channel[ 0 ]. data[ 0x08 ] & (1 << 13)) ? 1 : 0 ) == 1 ||
		  ( (_pArinc->channel[ 0 ]. data[ 0x08 ] & (1 << 14)) ? 1 : 0 ) == 1 ||
	    ( (_pArinc->channel[ 1 ]. data[ 0x08 ] & (1 << 13)) ? 1 : 0 ) == 1 ||
		  ( (_pArinc->channel[ 1 ]. data[ 0x08 ] & (1 << 14)) ? 1 : 0 ) == 1   )
		{
		active = activeState;
		}

	triggerOffCh1 = ( (_pArinc->channel[ 0 ]. data[ 0x0D ] & (1 << 14)) ? 1 : 0 );
	triggerOffCh2 = ( (_pArinc->channel[ 1 ]. data[ 0x0D ] & (1 << 14)) ? 1 : 0 );

	msTick = HAL_GetTick();

	if( msTick < _pArinc->channel[ 0 ].okTimeEnd )
		{
		if( triggerOffCh1Prev == 1 && triggerOffCh1 == 0 &&
				( (_pArinc->channel[ 0 ]. data[ 0xE4 ] >> 13) & 0xFFFF ) <= (0x0533) )
			{
			active = notActiveState;
			triggerOffCh1Prev = 0;
			triggerOffCh2Prev = 0;
			}
		}
	else if( msTick < _pArinc->channel[ 1 ].okTimeEnd )
		{
		if( triggerOffCh2Prev == 1 && triggerOffCh2 == 0 &&
				( (_pArinc->channel[ 1 ]. data[ 0xE4 ] >> 13) & 0xFFFF ) <= (0x0533) )
			{
			active = notActiveState;
			triggerOffCh1Prev = 0;
			triggerOffCh2Prev = 0;
	...
	if( triggerOffCh1 )
		triggerOffCh1Prev = triggerOffCh1;
	if( triggerOffCh2 )
		triggerOffCh2Prev = triggerOffCh2;
...
void CORE_threadHoltActiveChannels(void)
	...
	tdArincChannel *pChannelEnd;
	uint32_t time;
	tdChannelState channelState;
	uint8_t channel;
	...
	pChannel = &arinc.channel[ 0 ];
	pChannelEnd = &arinc.channel[ 0 ] + sizeof(arinc.channel) / sizeof(arinc.channel[ 0 ]);
	do
		{
		channelState = CORE_holtIsChannelActive( &arinc, pChannel, channel++ );

		if( pChannel->status.enable == 0 )
				continue;

		if( channelState != sameState )
			{
			pChannel->status.state = channelState;
			}


		if( pChannel->status.state != notActiveState )
			{
			active++;
			}
		else
...
#line 343 "..\\..\\Src\\core\\core.c"

		}	while( ++pChannel < pChannelEnd );
	...
	if( active != 0 && notActive == 0 )
		...
		do
			...
			pChannel->status.activeTimeEnd = time + 3000;
			}	while( ++pChannel < pChannelEnd );
		}
	...
	do
		...
		pChannel->status.active = ( time < pChannel->status.activeTimeEnd ) ? 1 : 0;
		
		if( pChannel->status.active )
			active++;

		}	while( ++pChannel < pChannelEnd );
	
	arinc.activeChannelsAmount = active;
	}
...
void CORE_threadGetData(void)
	...
	tdMemoryPage *pPage = &pagesQueue.page[ pagesQueue.pen % 4 ];

	time = HAL_GetTick();

	if( pPage->dataSize < ((unsigned int)(4096 / 5) * 5) )
		{
		HOLT_CORE_ReedAvailableData(&HOLT3598, &arinc, pPage->data, &pPage->dataSize, ((unsigned int)(4096 / 5) * 5));
		}

	if( prevDataSize == 0 && pPage->dataSize != 0 )
		{
		{ pPage ->spare[ 0 + 0 ] = ( (ARINC429 >> 0) & 0xFF ); };
		{ pPage ->spare[ 13 + 3 ] = ( (time >> 24) & 0xFF ); pPage ->spare[ 13 + 2 ] = ( (time >> 16) & 0xFF ); pPage ->spare[ 13 + 1 ] = ( (time >> 8) & 0xFF ); pPage ->spare[ 13 + 0 ] = ( (time >> 0) & 0xFF ); };
	...
	if( ((pPage->dataSize < ((unsigned int)(4096 / 5) * 5)) == 0) || (pPage->dataSize != 0 && arinc.activeChannelsAmount == 0) )
		{
		{ pPage ->spare[ 17 + 3 ] = ( (time >> 24) & 0xFF ); pPage ->spare[ 17 + 2 ] = ( (time >> 16) & 0xFF ); pPage ->spare[ 17 + 1 ] = ( (time >> 8) & 0xFF ); pPage ->spare[ 17 + 0 ] = ( (time >> 0) & 0xFF ); };
		{ pPage ->spare[ 21 + 3 ] = ( (crc32 >> 24) & 0xFF ); pPage ->spare[ 21 + 2 ] = ( (crc32 >> 16) & 0xFF ); pPage ->spare[ 21 + 1 ] = ( (crc32 >> 8) & 0xFF ); pPage ->spare[ 21 + 0 ] = ( (crc32 >> 0) & 0xFF ); };
...
void CORE_threadClearNextBlock(void)
	{
	static uint32_t prevBlock = 4096 + 1;
	nand_addr_t address;
...
void CORE_threadSaveToMemory(void)
	{
	tdMemoryPage *pPage = &pagesQueue.page[ pagesQueue.eye % 4 ];

	if( pPage->dataReady )
		{
		FILE_SYSTEM_writeDataToFile(FAT.pFirstFile, pPage->data, 4096, pPage->spare, 224);

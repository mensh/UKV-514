...
#line 3 "..\\..\\Src\\core\\holtDriver.c"
#line 4 "..\\..\\Src\\core\\holtDriver.c"
...
tdStatus HOLT_DRIVER_setConfiguration(tdSpiDevice *_pDevice, uint8_t _channel, uint16_t _configuration)
	{
	static uint8_t txBuff[ 1 + 2 ];
	static uint8_t rxBuff[ 1 + 2 ];

	txBuff[ 0 ] = (uint8_t)setConfiguration;
	txBuff[ 0 ] |= ( (_channel & 0x0F) << 4 );
	txBuff[ 1 ] = (_configuration >> 8) & 0xFF;
	txBuff[ 2 ] = (_configuration >> 0) & 0xFF;

	return HOLT_DRIVER_dataTransmition(_pDevice, txBuff, rxBuff, 1 + 2);
	}
...
tdStatus HOLT_DRIVER_readConfiguration(tdSpiDevice *_pDevice, uint8_t _channel, uint16_t *_pConfiguration)
	...
	status = HOLT_DRIVER_dataTransmition(_pDevice, txBuff, rxBuff, 1 + 2);
...
tdStatus HOLT_DRIVER_readFifoWord(tdSpiDevice *_pDevice, uint8_t _channel, uint32_t *_word)
	...
	memset(txBuff, 0x00, 1 + 4);

	txBuff[ 0 ] = (uint8_t)readRxFifo;
	txBuff[ 0 ] |= ( (_channel & 0x0F) << 4 );

	status = HOLT_DRIVER_dataTransmition(_pDevice, txBuff, rxBuff, 1 + 4);
...
tdStatus HOLT_DRIVER_readFifoData(tdSpiDevice *_pDevice, uint8_t _channel, uint8_t *_data)
	...
	status = HOLT_DRIVER_dataTransmition(_pDevice, txBuff, _data, 1 + 4);
...
tdStatus HOLT_DRIVER_readStatus(tdSpiDevice *_pDevice, tdHoltStatus *_pStatus)
	...
	if( status == Ok )
		{
		_pStatus->notEmpty = ~rxBuff[ 2 ];	
		_pStatus->full = rxBuff[ 1 ];			
		}
	else
		{
		_pStatus->notEmpty = 0x00;
		_pStatus->full = 0x00;
		}
...
tdStatus HOLT_DRIVER_writeLoopBackFifoWord(tdSpiDevice *_pDevice, uint8_t _speedHigh, uint32_t _word)
	...
	return HOLT_DRIVER_dataTransmition(_pDevice, txBuff, rxBuff, 1 + 4);
	}

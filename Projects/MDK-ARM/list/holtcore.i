...
#line 6 "..\\..\\Src\\core\\holtCore.c"

tdSpiDevice HOLT3598;
tdArincReceiverFaild holtReceiversStatus;
...
void HOLT_CORE_init(tdSpiDevice *_pDevice, tdArinc *_pArinc)
	...
	pHspi = &hspi3;
	hot3598ChipSelect.pGPIO = ((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0800));
	hot3598ChipSelect.pin = ((uint16_t)0x2000);


	_pArinc->activeChannelsAmount = 0;

	SPI_DRIVER_initDevice(&HOLT3598, pHspi, &hot3598ChipSelect);

	HOLT_CORE_holtSelfTest(_pDevice, &holtReceiversStatus);
	...
	HOLT_DRIVER_masterReset( &HOLT3598 );
	}
...
tdStatus HOLT_CORE_configurateChannel(tdSpiDevice *_pDevice, uint8_t _channel, uint16_t _configuration)
	...
	while( HOLT_DRIVER_setConfiguration(_pDevice, _channel, _configuration) == Busy ); 

	while( HOLT_DRIVER_readConfiguration(_pDevice, _channel, &readBackConfiguration) == Busy ); 
...
tdStatus HOLT_CORE_readWordsFormChannel(tdSpiDevice *_pDevice, tdArinc *_pArinc, uint8_t _channel, uint8_t _wordsToRead, uint8_t *_pBuffer, uint32_t *_pCurrentSize, uint32_t _maxSize)
	...
	while( _wordsToRead )
		...
		if( HOLT_DRIVER_readFifoWord(_pDevice, _channel, &word) == Ok )
			{
			pArincChannel->okTimeEnd = HAL_GetTick() + 2000;

			lable = _pArinc->reversedLabels[ word & (256 - 1) ]; 
			pArincChannel->data[ lable ] = word;

			if( pArincChannel->status.active == 1 )
				{
				*(_pBuffer + *_pCurrentSize + 0) = channel;
				 { _pBuffer[ *_pCurrentSize + 1 + 3 ] = ( (word >> 24) & 0xFF ); _pBuffer[ *_pCurrentSize + 1 + 2 ] = ( (word >> 16) & 0xFF ); _pBuffer[ *_pCurrentSize + 1 + 1 ] = ( (word >> 8) & 0xFF ); _pBuffer[ *_pCurrentSize + 1 + 0 ] = ( (word >> 0) & 0xFF ); };
...
tdStatus HOLT_CORE_ReedAvailableData(tdSpiDevice *_pDevice, tdArinc *_pArinc, uint8_t *_pBuffer, uint32_t *_pCurrentSize, uint32_t _maxSize)
	...
	tryes = 0;
	flagsStatus = (((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1400))->IDR & (((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0004)|((uint16_t)0x0008)|((uint16_t)0x0010)|((uint16_t)0x0020)|((uint16_t)0x0040)|((uint16_t)0x0080)));
	while( flagsStatus != 0 && tryes++ < 4 )
		{
		status = HOLT_CORE_handleFifoEvents(_pDevice, _pArinc, flagsStatus, 1, _pBuffer, _pCurrentSize, _maxSize);
		flagsStatus = (((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1400))->IDR & (((uint16_t)0x0001)|((uint16_t)0x0002)|((uint16_t)0x0004)|((uint16_t)0x0008)|((uint16_t)0x0010)|((uint16_t)0x0020)|((uint16_t)0x0040)|((uint16_t)0x0080)));
		}
...
tdStatus HOLT_CORE_generateReversedLabels(tdArinc *_pArinc)
	...
	uint16_t lable;
	uint32_t S, T;
	for( lable = 0; lable < 256; lable++ )
		{
		S = (lable * 0x02020202) & 0x84422010;
		T = (lable * 8) & 0x00000420;
		_pArinc->reversedLabels[ (((S + T) % 1023) & (256 - 1)) ] = (uint8_t)lable;
		}
...
tdStatus HOLT_CORE_holtSelfTest(tdSpiDevice *_pDevice, tdArincReceiverFaild *_pFailStats)
	{
	volatile tdStatus status;
	uint8_t channel;
	uint8_t test;
	uint8_t trys;
	uint8_t testSpeedHigh;
	void (*pCurrentCallback)(void);
	tdHoltStatus holtStatus;
	uint16_t currentConfiguration[ 8 ];
	uint32_t loopBackData;
	volatile uint8_t failureStatsus[ 2 ] = { 0x00, 0x00 };
	uint16_t testCfg[ 2 ] = { ((0 << 5) | (1 << 0)),
																					 ((0 << 5) | (0 << 0)) };
	...
	for( test = 0; test < 2; test++ )
		...
		for( channel = 0; channel < 8; channel++ )
			{
			if( HOLT_CORE_configurateChannel(_pDevice, channel + 1, testCfg[ test ]) != Ok )
				{
				failureStatsus[ test ] |= ( 1 << channel );
				}
			}

		testSpeedHigh = (testCfg[ test ] & (0 << 0)) ? 1 : 0;

		HOLT_DRIVER_writeLoopBackFifoWord(_pDevice, testSpeedHigh, 0x9DB6692A);

		holtStatus.notEmpty = 0;
		trys = 10;

		while( holtStatus.notEmpty == 0 && trys-- )
			{
		...
		for( channel = 0; channel < 8; channel++ )
			...
			if( holtStatus.notEmpty & ( 1 << channel ) )
				{
				status = HOLT_DRIVER_readFifoWord(_pDevice, channel + 1, &loopBackData);
				if( status != Ok || loopBackData != 0x9DB6692A )
					{
					failureStatsus[ test ] |= ( 1 << channel );
					}
				}
	...
	TIM_CORE_setCallback(&htim7, pCurrentCallback);
	for( channel = 0; channel < 8; channel++ )
		{
		if( HOLT_CORE_configurateChannel(_pDevice, channel + 1, currentConfiguration[ channel ]) != Ok )
			{
			failureStatsus[ 1 ] |= ( 1 << channel );
			failureStatsus[ 0 ] |= ( 1 << channel );
			}
		}

	if( _pFailStats != 0 )
		{
		_pFailStats->high = failureStatsus[ 1 ];
		_pFailStats->low = failureStatsus[ 0 ];
		}
